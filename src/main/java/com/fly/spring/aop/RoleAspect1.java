...
@Aspect
@Order(2)//一个连接点有多个切面时，使用 @Order 指定执行顺序，多个切面的aop实际上是应用了责任链模式
public class RoleAspect1 {
  ...
  public void before(Role role, int sort) {
    System.out.println("前置通知执行-1"+role+sort);
  }
  ...
  public void around(ProceedingJoinPoint pj) {
    System.out.println("环绕通知执行-1-before");
    try {
      pj.proceed();//不执行，则会覆盖原有方法
    } catch (Throwable throwable) {
      throwable.printStackTrace();
    }
    System.out.println("环绕通知执行-1-after");
  }
